...
interface ModeDuelsProps {
  ...
  initialActiveGame?: DuelGame | null;
}
...
export const ModeDuels: React.FC<ModeDuelsProps> = ({
  ...
  const [activeGame, setActiveGame] = useState<DuelGame | null>(initialActiveGame || null);
  ...
  const handleStartBotPractice = async () => {
    ...
    const joined = await joinDuelGame(newGame.shareCode, {
      ...
      equippedShieldId: botOpponent.shieldId
    } as any);
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto flex-wrap">
            <button
              ...
              title="Veure qui està connectat ara o cercar per correu"
              className="flex-1 sm:flex-initial py-2.5 px-3.5 bg-emerald-600/90 hover:bg-emerald-500 text-white font-black rounded-xl text-xs sm:text-sm border border-emerald-400/40 flex items-center justify-center gap-1.5 active:scale-95 transition-transform cursor-pointer min-h-[44px] shadow-md shadow-emerald-900/30"
            >
              <Users className="w-4 h-4" />
              <span>Opositors Actius</span>
            </button>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 sm:p-6 shadow-2xl space-y-5">
          ...
          <div className={`flex flex-col items-center justify-center py-4 relative ${currentQuestion ? 'hidden' : 'flex'}`}>
            ...
            <div className="mt-4 text-center">
              {!isMyTurn ? (
                <div className="px-4 py-2 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400 text-xs font-bold">
                  ⏳ És el torn del teu rival ({rivalName}). Rebràs el torn quan falli una resposta.
                </div>
              ) : activeGame.status === 'finished' ? (
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-md p-5 shadow-2xl space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-black text-white flex items-center gap-2">
                <span>⚔️</span>
                <span>Crear Sala de Duel 1v1</span>
              </h3>
              <button 
            ...
            <p className="text-xs text-slate-400">
              Es crearà una sala de joc. Podràs enviar el codi o enllaç al teu company d'oposició perquè s'uneixi quan vulgui.
            </p>
            ...
            <div className="pt-2 space-y-2">
              <button
                ...
                className="w-full py-2.5 bg-emerald-950/60 hover:bg-emerald-900/80 text-emerald-300 font-black rounded-xl text-xs border border-emerald-500/40 flex items-center justify-center gap-2 cursor-pointer min-h-[42px]"
              >
                <Users className="w-4 h-4 text-emerald-400" />
                <span>Reptar Opositor Connectat Ara / Cercar Correu</span>
              </button>

              <button
                onClick={handleCreateNewGame}
                className="w-full py-3 bg-gradient-to-r from-sky-500 to-blue-600 text-white font-black rounded-xl text-xs sm:text-sm active:scale-95 cursor-pointer min-h-[44px]"
              >
                CREAR SALA I JUGAR EL 1R TORN ➔
              </button>

              <button
                onClick={handleStartBotPractice}
                className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 text-amber-300 font-bold rounded-xl text-xs border border-slate-700 cursor-pointer min-h-[40px]"
              >
                Jugar contra Caporal_IA (Bot de Pràctica)
              </button>
            </div>
          </div>
        </div>
